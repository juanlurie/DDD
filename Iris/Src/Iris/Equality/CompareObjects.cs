...
namespace Iris.Equality
{
    ...
    public class CompareObjects
    {
        ...
        bool IgnoredByAttribute(PropertyInfo property)
        {
            foreach (Type attributeType in AttributesToIgnore)
            {
            ...
            string breadCrumb)
        {
            ...
            foreach (PropertyInfo info in currentProperties)
            {
                ...
                //If we should ignore it, skip it
                if (ElementsToIgnore.Contains(info.Name) || IgnoredByAttribute(info) || IgnoredByAttribute(info.PropertyType))
                    continue;
